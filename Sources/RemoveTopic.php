...
function removeTopics($topics, $decreasePostCount = true, $ignoreRecycling = false)
{
	...
	while ($row = mysql_fetch_assoc($request))
	{
		...
		$adjustBoards[$row['ID_BOARD']]['numPosts'] += $row['numReplies'] + ($row['approved'] ? $row['numTopics'] : 0);
		$adjustBoards[$row['ID_BOARD']]['unapprovedPosts'] += $row['unapprovedPosts'];

		// Add the topics to the right type.
		if ($row['approved'])
			$adjustBoards[$row['ID_BOARD']]['numTopics'] += $row['numTopics'];
		else
	...
	if ($row['ID_LAST_MSG'] == $message)
	{
		...
		db_query("
			...
			SET
				...
				unapprovedPosts = IF(unapprovedPosts = 0, 0, unapprovedPosts - 1)") . "
			WHERE ID_TOPIC = $row[ID_TOPIC]
			LIMIT 1", __FILE__, __LINE__);
	}
