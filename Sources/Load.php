...
function reloadSettings()
{
	...
	$space_chars = $utf8 ? (@version_compare(PHP_VERSION, '4.3.3') != -1 ? '\x{A0}\{AD}\x{2000}-\x{200F}\x{201F}\x{202F}\x{3000}\x{FEFF}' : pack('C*', 0xC2, 0xA0, 0xC2, 0xAD, 0xE2, 0x80, 0x80) . '-' . pack('C*', 0xE2, 0x80, 0x8F, 0xE2, 0x80, 0x9F, 0xE2, 0x80, 0xAF, 0xE2, 0x80, 0x9F, 0xE3, 0x80, 0x80, 0xEF, 0xBB, 0xBF)) : '\xA0';
...
function loadMemberContext($user)
{
	...
	$memberContext[$user] = array(
		...
		'hide_email' => $profile['email_address'] == '' || (!empty($modSettings['guest_hideContacts']) && $user_info['is_guest']) || (!empty($profile['hide_email']) && !allowedTo('moderate_forum') && $user_info['id'] != $profile['id_member']),
		'email_public' => empty($profile['hide_email']) && (empty($modSettings['guest_hideContacts']) || !$user_info['is_guest']),
