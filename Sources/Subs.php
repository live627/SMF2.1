...
function constructPageIndex($base_url, &$start, $max_value, $num_per_page, $flexible_start = false)
{
	...
	else
	{
		...
		if ($start > $num_per_page * ($PageContiguous + 1))
			$pageindex .= '<span style="font-weight: bold;" onclick="expandPages(this, \'' . ($flexible_start ? strtr($base_url, array('\'' => '\\\'')) : strtr($base_url, array('%' => '%%', '\'' => '\\\'')) . ';start=%1$d') . '\', ' . $num_per_page . ', ' . ($start - $num_per_page * $PageContiguous) . ', ' . $num_per_page . ');" onmouseover="this.style.cursor=\'pointer\';"> ... </span>';
	...
	while ($pos !== false)
	{
		...
		if ($smileys !== false && $tag === null && isset($itemcodes[substr($message, $pos + 1, 1)]) && substr($message, $pos + 2, 1) == ']' && !isset($disabled['list']) && !isset($disabled['li']))
		{
			...
			if ($pos2 !== false && ($pos2 <= $pos3 || $pos3 === false))
			{
				...
				$linebreaks = $pos2 !== false && strpos($message, '<br /><br />') == $pos2 ? true : false;

				$message = substr($message, 0, $pos2) . (!empty($matches[0]) && substr($matches[0], -1) == '[' ? '[/li]' : '[/li][/list]') . substr($message, $pos2);

				$open_tags[count($open_tags) - 2]['after'] = '</ul>' . ($linebreaks ? '<br /><br />' : '');
			}
