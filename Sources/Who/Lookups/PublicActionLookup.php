...
final class PublicActionLookup implements LookupInterface
{
	...
	{
		return isset($actions['action']) && !isset($actions['sa'])
			&& Lang::txtExists(
				'whoall_' . $actions['action'],
				file: 'Who',
			);
	}
	...
	public function getText(array $actions): string|array
	{
		...
		if (
			...
			&& Lang::txtExists(
				$this->preferred_prefix . $actions['action'],
				file: 'Who',
			)
		) {
