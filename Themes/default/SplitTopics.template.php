<?php
// Version: 2.0 Beta 2; SplitTopics
	...
	foreach ($context['topics'] as $topic)
		echo '
							<tr>
								...
								<td class="windowbg2" valign="middle">
									<a href="' . $scripturl . '?topic=' . $topic['id'] . '.0" rel="new_win">' . $topic['subject'] . '</a>
								</td>
								<td class="windowbg2" valign="middle">
	...
	if (!empty($context['polls']))
	{
		...
		foreach ($context['polls'] as $poll)
			echo '
						<tr>
							<td>
								<input type="radio" name="poll" value="' . $poll['id'] . '"' . ($poll['selected'] ? ' checked="checked"' : '') . ' class="check" /> ' . $poll['question'] . ' (' . $txt['topic'] . ': <a href="' . $scripturl . '?topic=' . $poll['topic']['id'] . '.0" rel="new_win">' . $poll['topic']['subject'] . '</a>)
							</td>
						</tr>';
		echo '
						<tr>
