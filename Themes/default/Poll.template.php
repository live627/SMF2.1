...
function template_main()
{
	...
	if ($context['can_moderate_poll'])
	{
		echo '
						<div class="poll_options">
							<ul class="poll_options">
								...
								<li>
									<label for="poll_expire">', $txt['poll_run'], '</label> <input type="text" name="poll_expire" id="poll_expire" size="2" value="', $context['poll']['expiration'], '" onchange="this.form.poll_hide[2].disabled = isEmptyText(this) || this.value == 0; if (this.form.poll_hide[2].checked) this.form.poll_hide[1].checked = true;" maxlength="4" class="input_text" /> ', $txt['poll_run_days'], '
								</li>
								<li>
		...
		if ($context['poll']['guest_vote_allowed'])
			echo '
								<li>
									<label for="poll_guest_vote"><input type="checkbox" id="poll_guest_vote" name="poll_guest_vote"', !empty($context['poll']['guest_vote']) ? ' checked="checked"' : '', ' class="input_check" /> ', $txt['poll_guest_vote'], '</label>
								</li>';
	...
	echo '
						<div class="poll_options">
							<ul class="poll_options">
								...
								<li>
									<input type="radio" name="poll_hide" id="poll_results_voted" value="1"', $context['poll']['hide_results'] == 1 ? ' checked="checked"' : '', ' class="input_radio" /> <label for="poll_results_voted">', $txt['poll_results_voted'], '</label>
								</li>
								<li>
									<input type="radio" name="poll_hide" id="poll_results_expire" value="2"', $context['poll']['hide_results'] == 2 ? ' checked="checked"' : '', empty($context['poll']['expiration']) ? 'disabled="disabled"' : '', ' class="input_radio" /> <label for="poll_results_expire">', $txt['poll_results_expire'], '</label>
								</li>
							</ul>
						</div>
					</fieldset>';
