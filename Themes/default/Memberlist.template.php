...
function template_main()
{
	...
	if (!empty($context['members']))
	{
		...
		<tr style="text-align: center;"', empty($member['sort_letter']) ? '' : ' id="letter' . $member['sort_letter'] . '"', '>
			...
			<td class="windowbg2">', $member['show_email'] == 'no' ? '' : '<a href="' . ($member['show_email'] == 'no_through_forum' ? $scripturl . '?action=emailuser;sa=email;uid=' . $member['id'] : 'mailto:' . $member['email']) . '"><img src="' . $settings['images_url'] . '/email_sm.gif" alt="' . $txt['email'] . '" title="' . $txt['email'] . ' ' . $member['name'] . '" /></a>', '</td>';

		if (!isset($context['disabled_fields']['website']))
			echo '
			<td class="windowbg">', $member['website']['url'] != '' ? '<a href="' . $member['website']['url'] . '" rel="new_win"><img src="' . $settings['images_url'] . '/www.gif" alt="' . $member['website']['title'] . '" title="' . $member['website']['title'] . '" /></a>' : '', '</td>';
