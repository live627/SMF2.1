<?php
// Version: 2.0 Alpha; Errors

// Important! Before editing these language files please read the text at the topic of index.spanish.php.

$txt['no_access'] = 'No est&aacute;s autorizado para entrar a esta secci&oacute;n';

$txt['mods_only'] = 'S&oacute;lo Moderadores pueden usar la funci&oacute;n de eliminado directo, por favor elimina este mensaje a trav&eacute;s de la funci&oacute;n modificar.';
$txt['no_name'] = 'No llenaste el campo nombre. Es requerido.';
$txt['no_email'] = 'No llenaste el campo email. Es requerido';
$txt['topic_locked'] = 'Este tema est&aacute; bloqueado, no est&aacute;s autorizado para publicar o modificar mensajes...';
$txt['no_password'] = 'Campo contrase&ntilde;a vac&iacute;o';
$txt['already_a_user'] = 'El nombre de usuario que trataste de registrar ya existe.';
$txt['cant_move'] = 'No est&aacute;s autorizado para mover temas...';
$txt['members_only'] = 'Este servicio es solo para usuarios...';
$txt['login_to_post'] = 'Para publicar debes haber ingresado. Si no tienes todav&iacute;a una cuenta, <a href="' . $scripturl . '?action=register">reg&iacute;strate</a>.';
$txt['passwords_dont_match'] = 'No coinciden las contrase&ntilde;as.';
$txt['register_to_use'] = 'Lo siento, debes estar registrado antes de usar esta funci&oacute;n.';
$txt['password_invalid_character'] = 'Car&aacute;cter inv&aacute;lido en la contrase&ntilde;a';
$txt['name_invalid_character'] = 'Car&aacute;cter inv&aacute;lido en el nombre.';
$txt['email_invalid_character'] = 'Car&aacute;cter inv&aacute;lido en el Email.';
$txt['username_reserved'] = 'El nombre de usuario que trataste de registrar contiene el nombre reservado\'%s\'. Por favor intenta otro nombre de usuario.';
$txt['numbers_one_to_nine'] = 'Este campo s&oacute;lo acepta n&uacute;meros del 0-9';
$txt['not_a_user'] = 'El usuario del cual tratas de ver su perfil no existe.';
$txt['not_a_topic'] = 'Este tema no existe en el foro.';
$txt['email_in_use'] = 'Esa direcci&oacute;n de email (%s) est&aacute; siendo usada por un usuario registrado. Si crees que es un error, ve a la p&aacute;gina de ingreso y usa la funci&oacute;n recordar contrase&ntilde;a con esa direcci&oacute;n.';

$txt['didnt_select_vote'] = 'No seleccionaste una opci&oacute;n del voto.';
$txt['poll_error'] = 'Esa encuesta no existe, o ha sido bloqueada, o intentaste votar m&aacute;s de una vez.';
$txt['members_only'] = 'Esta opci&oacute;n est&aacute; disponible solamente para usuarios registrados.';
$txt['locked_by_admin'] = 'Fue bloqueada por un administrador.  No la puedes desbloquear.';
$txt['not_enough_posts_karma'] = 'Lo siento, no tienes mensajes suficientes para modificar el karma - necesitas al menos %1$d.';
$txt['cant_change_own_karma'] = 'Lo siento, no est&aacute;s autorizado para modificar tu propio karma.';
$txt['karma_wait_time'] = 'Lo siento, no puedes repetir una acci&oacute;n de karma sin esperar %1$s %2$s.';
$txt['feature_disabled'] = 'Lo siento, esta funci&oacute;n ha sido desactivada.';
$txt['cant_access_upload_path'] = '&iexcl;No se puede accesar al directorio de los archivos adjuntos!';
$txt['file_too_big'] = 'Tu archivo es demasiado grande. El tama&ntilde;o m&aacute;ximo permitido para archivos adjuntos es %d KB.';
$txt['attach_timeout'] = 'Hubo un error al intentar guardar tu archivo adjunto, por favor intenta nuevamente.';
$txt['filename_exisits'] = '&iexcl;Lo siento! Ese nombre de archivo ya existe. Por favor renombra el archivo e intenta nuevamente.';
$txt['ran_out_of_space'] = 'El directorio de uploads est&aacute; lleno. Por favor intenta un archivo m&aacute;s peque&ntilde;o y contacta al administrador.';
$txt['couldnt_connect'] = 'No se pudo conectar al servidor o no se encontr&oacute; el archivo';
$txt['no_board'] = 'El foro que especificaste no existe';
$txt['cant_split'] = 'No est&aacute;s autorizado para dividir temas';
$txt['cant_merge'] = 'No est&aacute;s autorizado para combinar temas';
$txt['no_topic_id'] = 'Especificaste un ID de tema inv&aacute;lido.';
$txt['split_first_post'] = 'No puedes dividir un tema desde el primer mensaje.';
$txt['topic_one_post'] = 'Este tema s&oacute;lo contiene un mensaje y no puede ser dividido';
$txt['no_posts_selected'] = 'no se seleccionaron mensajes';
$txt['slected_all_posts'] = 'No se pudo dividir. Seleccionaste todos los mensajes.';
$txt['cant_find_messages'] = 'No se encontraron los mensajes';
$txt['cant_insert_topic'] = 'No se pudo insertar el tema';
$txt['already_a_mod'] = 'Has seleccionado el nombre de usuario de un moderador. Por favor selecciona otro nombre de usuario';
$txt['session_timeout'] = 'Falla al enviar el formulario. La sesi&oacute;n ha expirado.';
$txt['session_varify_fail'] = 'Falla al validar la sesi&oacute;n. Por favor intenta nuevamente.';
$txt['verify_url_fail'] = 'Sitio de referencia inv&aacute;lido.';

// Untranslated!
$txt['cannot_access_mod_center'] = 'You do not have permission to access the moderation center.';
$txt['cannot_admin_forum'] = 'No est&aacute;s autorizado para administrar este foro.';
$txt['cannot_announce_topic'] = 'No est&aacute;s autorizado para publicar anuncios en este foro.';
// Untranslated!
$txt['cannot_approve_posts'] = 'You do not have permission to approve items.';
$txt['cannot_post_unapproved_attachments'] = 'You do not have permission to post unapproved attachments.';
$txt['cannot_post_unapproved_topics'] = 'You do not have permission to post unapproved topics.';
$txt['cannot_post_unapproved_replies_own'] = 'You do not have permission to post unapproved replies to your topics.';
$txt['cannot_post_unapproved_replies_any'] = 'You do not have permission to post unapproved replies to other users\' topics.';
$txt['cannot_calendar_edit_any'] = 'No puedes editar los eventos del calendario.';
$txt['cannot_calendar_edit_own'] = 'No tienes los privilegios necesarios para editar tus propios eventos.';
$txt['cannot_calendar_post'] = 'La edici&oacute;n de eventos no est&aacute; permitida - Lo sentimos.';
$txt['cannot_calendar_view'] = 'Lo sentimos, pero no est&aacute;s autorizado para ver el calendario.';
$txt['cannot_remove_any'] = 'Lo sentimos, pero no tienes los privilegios para eliminar temas.';
$txt['cannot_remove_own'] = 'No puedes eliminar tus propios temas en este foro.';
$txt['cannot_edit_news'] = 'No est&aacute;s autorizado para editar los elementos de las noticias en este foro.';
$txt['cannot_pm_read'] = 'Lo sentimos, no est&aacute;s autorizado para leer tus mensajes personales.';
$txt['cannot_pm_send'] = 'No est&aacute;s autorizado para enviar mensajes personales.';
$txt['cannot_karma_edit'] = 'No estas autorizado para modificar el karma de otras personas.';
$txt['cannot_lock_any'] = 'No est&aacute;s autorizado para bloquear temas aqu&iacute;.';
$txt['cannot_lock_own'] = 'Lo sentimos, pero no puedes bloquear tus propios temas aqu&iacute;.';
$txt['cannot_make_sticky'] = 'No est&aacute;s autorizado para fijar este tema.';
$txt['cannot_manage_attachments'] = 'No est&aacute;s autorizado para administrar archivos adjuntos o avatares.';
$txt['cannot_manage_bans'] = 'No est&aacute;s autorizado para cambiar la lista de accesos prohibidos.';
$txt['cannot_manage_boards'] = 'No est&aacute;s autorizado para administrar foros y categor&iacute;s.';
// Untranslated!
$txt['cannot_manage_membergroups'] = 'You cannot manage groups.';
$txt['cannot_manage_membergroups'] = 'No tienes los permisos necesarios para modificar o asignar grupos de usuarios.';
$txt['cannot_manage_permissions'] = 'No tienes los permisos necesarios para administrar los permisos.';
$txt['cannot_manage_smileys'] = 'No est&aacute;s autorizado para administrar smileys.';
$txt['cannot_mark_any_notify'] = 'No tienes los permisos necesarios para obtener notificaciones de este tema.';
$txt['cannot_mark_notify'] = 'Lo sentimos, pero no puedes solicitar notificaciones desde este foro.';
$txt['cannot_merge_any'] = 'No est&aacute;s autorizado para combinar temas en este foro.';
$txt['cannot_moderate_forum'] = 'No est&aacute;s autorizado para moderar este foro.';
$txt['cannot_modify_any'] = 'No est&aacute;s autorizado para modificar mensajes.';
$txt['cannot_modify_own'] = 'Lo sentimos, pero no est&aacute;s autorizado para editar tus propios mensajes.';
$txt['cannot_modify_replies'] = 'A pesar que este mensaje es en respuesta a tu tema, no lo puedes editar.';
// Untranslated!
$txt['cannot_move_own'] = 'You are not allowed to move your own topics in this board.';
$txt['cannot_move_any'] = 'No est&aacute;s autorizado para mover temas en este foro.';
$txt['cannot_poll_add_own'] = 'Lo sentimos, pero no est&aacute;s autorizado para agregarle encuestas a tus temas en este foro.';
$txt['cannot_poll_add_any'] = 'No puedes agregarle encuestas a este tema.';
$txt['cannot_poll_edit_own'] = 'No puedes editar esta encuesta, aunque sea tuya.';
$txt['cannot_poll_edit_any'] = 'Se te ha denegado el acceso para editar encuestas en este foro.';
$txt['cannot_poll_lock_own'] = 'No est&aacute;s autorizado para bloquear tus propias encuestas en este foro.';
$txt['cannot_poll_lock_any'] = 'Lo sentimos, pero no est&aacute;s autorizado para bloquear cualquier encuesta.';
$txt['cannot_poll_post'] = 'No est&aacute;s autorizado para publicar encuestas en el foro actual.';
$txt['cannot_poll_remove_own'] = 'No tienes los permisos para eliminar esta encuesta de tu tema.';
$txt['cannot_poll_remove_any'] = 'No puedes eliminar encuestas en este foro.';
$txt['cannot_poll_view'] = 'No est&aacute;s autorizado para ver encuestas en este foro.';
$txt['cannot_poll_vote'] = 'Lo sentimos, pero no puedes votar en las encuestas de este foro.';
$txt['cannot_post_attachment'] = 'No tienes los permisos necesarios para subir archivos adjuntos aqu&iacute;.';
$txt['cannot_post_new'] = 'Lo sentimos, pero no puedes publicar nuevos temas en este foro.';
$txt['cannot_post_reply_any'] = 'No tienes los permisos necesarios para publicar respuestas a temas en este foro.';
$txt['cannot_post_reply_own'] = 'No est&aacute;s autorizado para publicar respuestas a&uacute;n para tus propios temas en este foro.';
$txt['cannot_profile_remove_own'] = 'Lo sentimos, pero no est&aacute;s autorizado para borrar tu cuenta.';
$txt['cannot_profile_remove_any'] = '&iquest;No tienes los permisos necesarios para eliminar las cuentas de otras personas!';
$txt['cannot_profile_extra_any'] = 'No tienes los permisos necesarios modificar la configuraci&oacute;n de otros perfiles.';
$txt['cannot_profile_identity_any'] = 'No est&aacute;s autorizado para modificar la configuraci&oacute;n de otras cuentas.';
$txt['cannot_profile_title_any'] = 'No puedes editar los t&iacute;tulos personalizados de otras personas.';
$txt['cannot_profile_extra_own'] = 'Lo sentimos, pero no tienes los permisos necesarios para editar los datos de tu perfil.';
$txt['cannot_profile_identity_own'] = 'Por el momento, no puedes cambiar tu identidad.';
$txt['cannot_profile_title_own'] = 'No est&aacute;s autorizado para cambiar tu t&iacute;tulo personalizado.';
// Untranslated!
$txt['cannot_profile_server_avatar'] = 'You are not permitted to use a server stored avatar.';
$txt['cannot_profile_upload_avatar'] = 'You do not have permission to upload an avatar.';
$txt['cannot_profile_remote_avatar'] = 'No tienes el privilegio de usar un avatar remoto.';
$txt['cannot_profile_view_own'] = 'Lo sentimos, pero no puedes ver tu propio perfil.';
$txt['cannot_profile_view_any'] = 'Lo sentimos, pero no puedes ver el perfil de otras personas.';
$txt['cannot_delete_own'] = 'No est&aacute;s, en este foro, autorizado para borrar tus propios mensajes.';
$txt['cannot_delete_replies'] = 'Lo sentimos, pero no puedes eliminar estos mensajes, a&uacute;n cuando son respuestas a tu tema.';
$txt['cannot_delete_any'] = 'No est&aacute; permitido borrar mensajes en este foro.';
$txt['cannot_report_any'] = 'No est&aacute;s autorizado para reportar mensajes en este foro.';
$txt['cannot_search_posts'] = 'No est&aacute;s autorizado a buscar mensajes en este foro.';
$txt['cannot_send_mail'] = 'No tienes el privilegio de enviar mails.';
// Untranslated!
$txt['cannot_issue_warning'] = 'Sorry, you do not have permission to issue warnings to members.';
$txt['cannot_send_topic'] = 'Lo sentimos, pero el administrador ha desactivado la opci&oacute;n de enviar temas en este foro.';
$txt['cannot_split_any'] = 'No est&aacute; permitido dividir temas en este foro.';
$txt['cannot_view_attachments'] = 'Parece que no est&aacute;s autorizado para descargar o ver archivos adjuntos en este foro.';
$txt['cannot_view_mlist'] = 'No puedes ver la lista de usuarios, ya que no tienes permisos para ello.';
$txt['cannot_view_stats'] = 'No est&aacute;s autorizado para ver las estad&iacute;sticas del foro.';
$txt['cannot_who_view'] = 'Lo sentimos - no tienes los permisos necesarios para ver la lista de usuarios en l&iacute;nea.';

$txt['no_theme'] = 'Ese tema no existe.';
$txt['theme_dir_wrong'] = 'El directorio de default de los temas es incorrecto, favor de corregirlo haciendo <i>click</i> en este texto.';
$txt['registration_disabled'] = 'Lo sentimos, el registro de usuarios est&aacute; actualmente deshabilitado.';
$txt['registration_no_secret_question'] = 'Lo sentimos, no se ha especificado pregunta secreta para este usuario.';
$txt['poll_range_error'] = 'Lo sentimos, la encuesta debe durar m&aacute;s de 0 d&iacute;as.';
$txt['delFirstPost'] = 'No est&aacute;s autorizado para borrar el primer post de este tema.<p>Si deseas borrar este tema, haz click en \'Eliminar Tema\', o p&iacute;dele a un Administrador/Moderador para que lo borre por t&iacute;.</p>';
$txt['parent_error'] = '&iexcl;No se pudo crear el foro!';
$txt['login_cookie_error'] = 'No pudiste ingresar.  Favor de revisar la configuraci&oacute;n de tus cookies.';
$txt['incorrect_answer'] = 'Lo sentimos, pero no contestaste correctamente tu pregunta secreta.  Haz click en el bot&oacute;n Atr&aacute;s para intentar nuevamente, o haz click en el bot&oacute;n Atr&aacute;s 2 veces para usar el m&eacute;todo por defecto para obtener tu contrase&ntilde;a.';
$txt['no_mods'] = '&iexcl;No se encontraron moderadores!';
$txt['parent_not_found'] = 'Estructura corrupta del foro: no se pudo encontrar el foro padre';
$txt['modify_post_time_passed'] = 'No puedes modificar este mensaje ya que el tiempo l&iacute;mite permitido para editar mensajes ha transcurrido.';

$txt['calendar_off'] = 'El calendario est&aacute; deshabilitado, no puedes accesarlo en este momento.';
$txt['invalid_month'] = 'Mes inv&aacute;lido.';
$txt['invalid_year'] = 'A&ntilde;o inv&aacute;lido.';
$txt['event_month_missing'] = 'Falta el mes del evento.';
$txt['event_year_missing'] = 'Falta el a&ntilde;o del evento.';
$txt['event_day_missing'] = 'Falta el d&iacute;a del evento.';
$txt['event_title_missing'] = 'Falta el t&iacute;tulo del evento.';
$txt['invalid_date'] = 'Fecha inv&aacute;lida.';
$txt['no_event_title'] = 'No se especific&oacute; el t&iacute;tulo del evento.';
$txt['missing_event_id'] = 'Falta el ID del evento.';
$txt['cant_edit_event'] = 'No estas autorizado para modificar este evento.';
$txt['missing_board_id'] = 'Falta ID del foro.';
$txt['missing_topic_id'] = 'Falta ID del tema.';
$txt['topic_doesnt_exist'] = 'No existe el tema.';
$txt['not_your_topic'] = 'No eres el due&ntilde;o de este tema.';
$txt['board_doesnt_exist'] = 'El foro no existe.';
$txt['no_span'] = 'La caracter&iacute;stica de expansi&oacute;n est&aacute; deshabilitada.';
$txt['invalid_days_numb'] = 'N&uacute;mero inv&aacute;lido de d&iacute;as para expandirse.';

$txt['moveto_noboards'] = '&iexcl;No hay foros a donde mover este tema!';

$txt['already_activated'] = 'Tu cuenta ya ha sido previamente activada.';
$txt['still_awaiting_approval'] = 'Tu cuenta est&aacute; todav&iacute;a en espera de la aprovaci&oacute;n de un administrador.';

$txt['invalid_email'] = 'Email inv&aacute;lido / rango del email.<br />Ejemplo de una direcci&oacute;n de email v&aacute;lida: webmaster@HablaJapones.org.<br />Ejemplo de un rango de direcci&oacute;n email v&aacute;lido: *@*.HablaJapones.org';
$txt['invalid_expiration_date'] = 'La fecha de expiraci&oacute;n no es v&aacute;lida';
$txt['invalid_hostname'] = 'Nombre del dominio inv&aacute;lido/ rango del dominio.<br />Ejemplo de un nombre de dominio v&aacute;lido: proxy4.HablaJapones.org<br />Ejemplo de un rango de dominio v&aacute;lido: *.HablaJapones.org';
$txt['invalid_ip'] = 'IP inv&aacute;lido / rango de IPs.<br />Ejemplo de una direcci&oacute;n IP v&aacute;lida: 127.0.0.1<br />Ejemplo de un rango v&aacute;lido de direcciones IP: 127.0.0-20.*';
$txt['invalid_username'] = 'Nombre del usuario no encontrado';
$txt['no_ban_admin'] = '&iexcl;No puedes prohibirle el acceso a un administrador - Debes degradarlo primero!';
$txt['no_bantype_selected'] = 'No se seleccion&oacute; el tipo prohibici&oacute;n de acceso';
$txt['ban_not_found'] = 'No se encontr&oacute; esta restricci&oacute;n de acceso';
$txt['ban_unknown_restriction_type'] = 'Tipo de restricci&oacute;n desconocida';
$txt['ban_name_empty'] = 'The name of the ban was left empty';
$txt['ban_name_exists'] = 'The name of this ban already exists. Please chose a different name.';

$txt['recycle_no_valid_board'] = 'No se ha seleccionado un foro v&aacute;lido para reciclar temas';

$txt['login_threshold_fail'] = 'Lo sentimos, por el momento no cuentas con intentos para ingresar.  Favor de intentar nuevamente m&aacute;s tarde.';

$txt['who_off'] = 'No puedes accesar la lista de usuarios en l&iacute;nea por el momento, ya que est&aacute; deshabilitada.';

$txt['merge_create_topic_failed'] = 'Error al intentar crear un nuevo tema.';
$txt['merge_need_more_topics'] = 'El combinar temas necesita de al menos 2 temas para combinar.';

$txt['spamWaitTime_broken'] = 'La &uacute;ltima publicaci&oacute;n desde tu IP se recibi&oacute; hace menos que %d segundos. Por favor int&eacute;ntalo m&aacute;s tarde.';
$txt['registerWaitTime_broken'] = '&iexcl;Te has registrado hace apenas %d segundos!';
$txt['loginWaitTime_broken'] = 'Tendr&aacute;s que esperar aproximadamente %d segundos para ingresar nuevamente.';

$txt['topic_gone'] = 'El tema o foro que est&aacute;s buscando parece que no existe, o fuera de tus l&iacute;mites.';
$txt['theme_edit_missing'] = '&iexcl;El archivo que intentas editar... no puede encontrarse!';

$txt['attachments_no_write'] = 'No puede escribirse en el directorio para subir archivos adjuntos.  Tu archivo adjunto o avatar no se pudo guardar.';
$txt['attachments_limit_per_post'] = 'No puedes subir m&aacute;s de %d archivos adjuntos por mensaje';

$txt['rtm_not_own'] = '&iexcl;No puedes reportar tus propios mensajes al moderador, eso no tiene sentido!';

$txt['no_dump_database'] = 'iexcl;Solamente los administradores pueden hacer respaldos de la base de datos!';
$txt['pm_not_yours'] = 'El mensaje personal que est&aacute;s tratando de citar no es tuyo o no existe, por favor regresa e intenta nuevamente.';
// Untranslated!
$txt['pm_too_many_recipients'] = 'You may not send personal messages to more than %d recipient(s) at once.';
$txt['pm_too_many_per_hour'] = 'You have exceeded the limit of %d personal messages per hour.';
$txt['labels_too_many'] = '&iexcl;Lo sentimos, %s mensajes ya tienen el n&uacute;mero m&aacute;ximo de etiquetas permitido!';
$txt['mangled_post'] = 'Datos de la forma destrozados - por favor regresa e intenta nuevamente.';
// Untranslated!
$txt['quoted_post_deleted'] = 'The post you are trying to quote either does not exist, was deleted, or is no longer viewable by you.';

$txt['register_only_once'] = 'Lo sentimos, pero no tienes permitido el registrar m&uacute;ltiples cuentas al mismo tiempo desde la misma computadora.';
$txt['admin_setting_coppa_require_contact'] = 'Debes proporcionar una direcci&oacute;n o fax de contacto si la aprobaci&oacute;n de los padres/tutor es requerida.';

$txt['error_long_name'] = 'El nombre que utilizaste fue muy largo.';
$txt['error_no_name'] = 'No se proporcion&oacute; ning&uacute;n nombre.';
$txt['error_bad_name'] = 'El nombre que enviaste no se pudo utilizar, ya que es un nombre reservado.';
$txt['error_no_email'] = 'No se proporcion&oacute; ninguna direcci&oacute;n de email.';
$txt['error_bad_email'] = 'Se proporcion&oacute; una direcci&oacute;n de email inv&aacute;lida.';
$txt['error_no_event'] = 'No se proporcion&oacute; un nombre de evento.';
$txt['error_no_subject'] = 'No se llen&oacute; el asunto.';
$txt['error_no_question'] = '&iexcl;La encuesta debe tener una pregunta!';
$txt['error_no_message'] = 'El cuerpo del mensaje se dej&oacute; vac&iacute;.';
$txt['error_long_message'] = 'El mensaje excede la longit&uacute;d m&aacute;xima permitida (%1$d caracteres).';
$txt['error_session_timeout'] = 'Tu sesi&oacute;n ha expirado. Favor de reenviar tu mensaje.';
$txt['error_no_to'] = 'No se especificaron destinatarios.';
$txt['error_bad_to'] = 'No se encontraron uno o mas destinatarios-\'para\'.';
$txt['error_bad_bcc'] = 'No se encontraron uno o mas destinatarios-\'bcc\'.';
$txt['error_form_already_submitted'] = '&iexcl;Ya haz enviado previamente este mensaje!  Probablemente hiciste doble-click, o intentaste refrescar el navegador.';
$txt['error_poll_few'] = '&iexcl;Debes tener al menos dos opciones!';
// Untranslated!
$txt['error_wrong_verification_code'] = 'The letters you typed don\'t match the letters that were shown in the picture.';
$txt['error_bad_file'] = 'Sorry but the file specified could not be opened: %s';
$txt['error_bad_line'] = 'The line you specified is invalid.';

$txt['smiley_not_found'] = 'No se encontr&oacute; el smiley.';
$txt['smiley_has_no_code'] = 'No se proporcion&oacute; c&oacute;digo para este smiley.';
$txt['smiley_has_no_filename'] = 'No se proporcion&oacute; el nombre del archivo para este smiley.';
$txt['smiley_not_unique'] = 'Un smiley con ese c&oacute;digo ya existe previamente.';
$txt['smiley_set_already_exists'] = 'Un smiley con ese URL ya existe previamente';
$txt['smiley_set_not_found'] = 'No se encontr&oacute; esa colecci&oacute;n de smileys';
$txt['smiley_set_path_already_used'] = 'El URL para esa colecci&oacute;n de smileys esta siendo utilizado actualmente por otra colecci&oacute;n de smileys.';
$txt['smiley_set_unable_to_import'] = 'No se pudo importar la colecci&oacute;n de smileys. El directorio es inv&aacute;lido o no pudo ser accesado.';

$txt['smileys_upload_error'] = 'No se pudo subir el archivo.';
$txt['smileys_upload_error_blank'] = '&iexcl;Todos los conjuntos de smileys deben tener una imagen!';
$txt['smileys_upload_error_name'] = '&iexcl;Todos los smileys deben tener el mismo nombre de archivo!';
$txt['smileys_upload_error_illegal'] = 'Tipo Ilegal.';

$txt['search_invalid_weights'] = 'Buscar pesos que no est&eacute;n configurados adecuadamente. Al menos un peso debe estar configurado para que sea diferente de cero. Favor de reportar este error a un administrador.';
$txt['unable_to_create_temporary'] = 'La funci&oacute;n de b&uacute;squeda no pudo crear tablas temporales.  Favor de intentar nuevamente.';

$txt['package_no_file'] = '&iexcl;No se pudo encontrar el archivo!';
$txt['packageget_unable'] = 'No fue posible conectarse al servidor.  Favor de intentar usando <a href="%s" target="_blank">este url</a>.';
$txt['not_on_simplemachines'] = 'Lo sentimos, los paquetes solo pueden ser descargados de esta manera del servidor de simplemachines.org.';
$txt['package_cant_uninstall'] = 'Este paquete o nunca fue instalado, o ya ha sido desinstalado previamente - no puedes desinstalarlo ahora.';
$txt['package_cant_download'] = '&iexcl;No puedes descargar o instalar nuevos paquetes ya que no se puede escribir en el directorio &quot;Packages&quot;!';
$txt['package_upload_error'] = '&iexcl;No se pudo subir el paquete, favor de revisar los permisos del directorio!';
$txt['package_upload_error_exists'] = 'El archivo que est&aacute;s subiendo ya existe en el servidor. Favor de borrarlo e intentar nuevamente.';
$txt['package_upload_error_supports'] = 'Actualmente el manejador de paquetes s&oacute;lo permite estos tipos de archivos: %1$s.';
$txt['package_upload_error_broken'] = 'El paquete que intentaste subir no es un paquete v&aacute;lido o est&aacute; corrupto.';

$txt['no_membergroup_selected'] = 'No se seleccion&oacute; ning&uacute;n grupo de usuarios';
// Untranslated!
$txt['membergroup_does_not_exist'] = 'The membergroup doesn\'t exist or is invalid.';

$txt['at_least_one_admin'] = '&iexcl;Debe haber por lo menos un administrador en el foro!';

// Don't use entities in the below string.
$txt['attachment_not_found'] = 'No se encontr&oacute; el archivo adjunto';

$txt['error_no_boards_selected'] = '&iexcl;No se seleccion&oacute; ning&uacute;n foro v&aacute;lido!';
$txt['error_invalid_search_string'] = '&iquest;Olvidaste escribir lo que se buscar&aacute;?';
// Untranslated! - changed!
$txt['error_query_not_specific_enough'] = 'La consulta no es lo suficientemente espec&iacute;fica. Favor de utilizar palabras m&aacute;s largas.';
$txt['error_no_messages_in_time_frame'] = 'No se encontraron mensajes en el tiempo que se especific&oacute;';
// Untranslated!
$txt['error_no_labels_selected'] = 'No labels were selected!';

$txt['profile_errors_occurred'] = 'Ocurrieron los siguientes errores mientras se intentaba guardar tu perfil';
$txt['profile_error_bad_offset'] = 'La diferencia horaria est&aacute; fuera de rango';
$txt['profile_error_no_name'] = 'El campo del nombre se dej&oacute; en blanco';
$txt['profile_error_name_taken'] = 'El nombre de usuario especificado ya est&aacute; siendo utilizado por alguien m&aacute;s';
$txt['profile_error_no_email'] = 'El campo de email se dej&oacute; en blanco';
$txt['profile_error_bad_email'] = 'No has proporcionado una direcci&oacute;n de email v&aacute;lida';
$txt['profile_error_email_taken'] = 'Otro usuario ya se encuentra registrado con esa direcci&oacute;n de email';
$txt['profile_error_no_password'] = 'No especificaste tu contrase&ntilde;a';
$txt['profile_error_bad_new_password'] = 'Las nuevas contrase&ntilde;as que especificaste no coinciden';
$txt['profile_error_bad_password'] = 'La contrase&ntilde;a que proporcionaste es incorrecta';
$txt['profile_error_bad_avatar'] = 'El avatar que has seleccionado es muy grande';
// Untranslated!
$txt['profile_error_password_short'] = 'Your password must be at least ' . (empty($modSettings['password_strength']) ? 4 : 8) . ' characters long.';
$txt['profile_error_password_restricted_words'] = 'Your password must not contain your username, email address or other commonly used words.';
$txt['profile_error_password_chars'] = 'Your password must contain a mix of upper and lower case letters, as well as digits.';
$txt['profile_error_already_requested_group'] = 'You already have an outstanding request for this group!';

$txt['mysql_error_space'] = ' - revisa el espacio disponible de la base de datos.';

$txt['icon_not_found'] = 'El icono no se pudo encontrar en el tema default - por favor revisa que la imagen ha sido subida a su respectivo directorio e intenta nuevamente.';
$txt['icon_after_itself'] = '&iexcl;El icono no puede colocarse despu&eacute;s de s&iacute; mismo!';
// Untranslated!
$txt['icon_name_too_long'] = 'Icon filenames cannot be more than 16 characters long';

$txt['name_censored'] = 'Lo sentimos, el nombre que intentaste usar, %s, contiene palabras que han sido censuradas.  Por favor intenta con otro nombre.';

// Untranslated!
$txt['poll_already_exists'] = 'A topic can only have one poll associated with it!';
$txt['poll_not_found'] = 'There is no poll associated with this topic!';

// Untranslated!
$txt['error_while_adding_poll'] = 'The following error or errors occurred whilst adding this poll';
$txt['error_while_editing_poll'] = 'The following error or errors occurred whilst editing this poll';

// Untranslated!
$txt['loadavg_search_disabled'] = 'Due to high stress on the server, the search function is temporarily disabled.';
$txt['loadavg_generic_disabled'] = 'Sorry, because of high stress on the server, this feature is currently unavailable.';
$txt['loadavg_allunread_disabled'] = 'The server\'s resources are temporarily under too high a demand to find all the topics you have not read.';
$txt['loadavg_unreadreplies_disabled'] = 'The server is currently under high stress.  Please try again shortly.';
$txt['loadavg_show_posts_disabled'] = 'Please try again later.  This member\'s posts are not currently available due to high load on the server.';

// Untranslated!
$txt['cannot_edit_permissions_inherited'] = 'You can not edit inherited permissions directly, you must either edit the parent group or edit the membergroup inheritance.';
// Untranslated!
$txt['mc_no_modreport_specified'] = 'You need to specify which report you wish to view.';
$txt['mc_no_modreport_found'] = 'The specified report either doesn\'t exist or is off limits to you';
$txt['st_cannot_retrieve_file'] = 'Could not retrieve the file %1$s.';
$txt['admin_file_not_found'] = 'Could not load the requested file: %1$s.';
$txt['themes_none_selectable'] = 'At least one theme must be selectable.';
$txt['themes_default_selectable'] = 'The overall forum default theme must be a selectable theme.';
$txt['ignoreboards_disallowed'] = 'The option to ignore boards has not been enabled.';

$txt['mboards_delete_error'] = '&iexcl;No se seleccion&oacute; ninguna categor&iacute;a!';
$txt['mboards_delete_board_error'] = '&iexcl;No se seleccion&oacute; ning&uacute;n foro!';

$txt['smileys_upload_error_notwritable'] = 'Los siguientes directorios de smileys no son escribibles: %1$s';
$txt['smileys_upload_error_types'] = 'La Imagen solamente puede tener las siguientes extensiones: %1$s.';

$txt['change_email_success'] = 'Tu direcci&oacute;n de email ha sido cambiada, y se le ha enviado un nuevo email de activaci&oacute;n.';
$txt['resend_email_success'] = 'Un nuevo email de activaci&oacute;n ha sido enviado satisfactoriamente.';

// Untranslated!
$txt['custom_option_need_name'] = 'The profile option must have a name!';
$txt['custom_option_not_unique'] = 'Field name is not unique!';

// Untranslated!
$txt['warning_no_reason'] = 'You must enter a reason for altering the warning state of a member.';
$txt['warning_notify_blank'] = 'You selected to notify the user but did not fill in the subject/message fields.';

?>