...
FROM {$from_prefix}users AS m
	LEFT JOIN {$from_prefix}kunena_users AS fb ON (fb.userid = m.id)
...
FROM {$from_prefix}kunena_categories
WHERE parent = 0;
---*
...
FROM {$from_prefix}kunena_messages AS t
	INNER JOIN {$from_prefix}kunena_messages AS m ON (m.thread = t.id)
WHERE t.parent = 0
GROUP BY t.id
...
FROM {$to_prefix}topics AS t
	INNER JOIN {$from_prefix}kunena_messages AS m ON (m.thread = t.id_last_msg);
---*
...
FROM {$from_prefix}kunena_messages AS m
	INNER JOIN {$from_prefix}kunena_messages_text AS mt ON (mt.mesid = m.id);
---*
...
FROM {$from_prefix}kunena_subscriptions;
---*
...
FROM {$from_prefix}kunena_moderation;
---*
...
$file = $path_from . '/' . $row['folder'] . '/' . $row['filename'];
$file_hash = getAttachmentFilename(basename($file), $id_attach, null, true);
$physical_filename = $id_attach . '_' . $file_hash;

if (strlen($physical_filename) > 255)
	return;
if (copy($file, $attachmentUploadDir . '/' . $physical_filename))
{
	@touch($attachmentUploadDir . '/' . $physical_filename, filemtime($file));
	$rows[] = array(
		'id_attach' => $id_attach,
...
$file = $path_from . '/media/kunena/avatars/' . $row['filename'];
