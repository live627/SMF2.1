...
FROM {$from_prefix}users AS m
	LEFT JOIN {$from_prefix}fb_users AS fb ON (fb.userid = m.id)
...
FROM {$from_prefix}fb_subscriptions;
---*
...
FROM {$from_prefix}fb_moderation;
---*
...
FROM {$from_prefix}fb_attachments;
---*
