...
if (empty($preparsing))
{
	...
	function convertStep1()
	{
		...
		// Initialize the session for cstep
		$_SESSION['cstep'] = 1;
	}
	...
	function convertStep2()
	{
		...
		while ($entry = $dir->read())
		{
			...
			$row = array(
				...
				'location' => isset($data['locaiton']) ? htmlspecialchars($data['location']) : '',
				...
				'last_login' => isset($data['lastonline']) ? $data['lastonline'] : '',
	...
	function convertStep4()
	{
		...
		if ($_GET['substep'] == 0 || (isset($_SESSION['cstep']) && $_SESSION['cstep'] < 4))
		{
			convert_query("
	...
	function convertStep5()
	{
		...
		if ($_GET['substep'] == 0 || (isset($_SESSION['cstep']) && $_SESSION['cstep'] < 5))
		{
			convert_query("
		...
		{
			$row = array(
				'name' => substr($cat['name'], 3, 255),
				'cat_order' => @$cat_order[$tempID],
				'tempID' => strtolower($tempID),
			);
		...
		foreach ($board_data as $line)
		{
			...
			$tempCatID = strtolower($tempCatID);

			$row = array(
				'name' => substr($boards[$tempID], 3, 255),
	...
	function convertStep6()
	{
		...
		if ($_GET['substep'] == 0 || (isset($_SESSION['cstep']) && $_SESSION['cstep'] < 6))
		{
			convert_query("
	...
	function convertStep7()
	{
		...
		if ($_GET['substep'] == 0 || (isset($_SESSION['cstep']) && $_SESSION['cstep'] < 7))
		{
			convert_query("
	...
	function convertStep8()
	{
		...
		if ($_GET['substep'] == 0 || (isset($_SESSION['cstep']) && $_SESSION['cstep'] < 8))
		{
			convert_query("
				ALTER TABLE {$to_prefix}boards
				DROP COLUMN tempID");
	...
	function convertStep10()
	{
		...
		if ($_GET['substep'] == 0 || (isset($_SESSION['cstep']) && $_SESSION['cstep'] < 10))
		{
	...
	function convertStep11()
	{
		...
		// Kill the session since this is the last steps that needs it
		unset($_SESSION['cstep']);
	}
	...
	function doBlock($table, &$block)
	{
		...
		if ($table == 'log_mark_read' || $table == 'log_boards' || $table == 'log_topics')
			$ignore = ' IGNORE ';
		else
			$ignore = '';
