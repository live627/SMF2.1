...
{
	global $modSettings, $user_settings, $context, $database, $my, $acl;
	global $db_name, $user_info, $mosConfig_sitename, $mosConfig_live_site;
	global $mosConfig_sef, $mosConfig_useractivation;
	global $mosConfig_allowUserRegistration, $mosConfig_mailfrom;
	global $mosConfig_fromname, $mosConfig_mailfrom, $mosConfig_fromname;
	global $mosConfig_dbprefix,$db_prefix, $pm_on_reg, $smf_path;
	global $use_realname, $sourcedir, $mosConfig_db, $cb_reg;
...
function resend_activation($option)
{
	...
	$database->setQuery("
		SELECT activation , username, name
		FROM {$mosConfig_dbprefix}users
		WHERE username = '$checkusername'
			AND email = '$confirmEmail'
		LIMIT 1");
